...
object OrderedMap {
  def empty[A, B]                 = new OrderedMap()
  def apply[A, B](elems: (A, B)*) = new OrderedMap(Map(elems: _*), Queue(elems: _*))
}
/**
 * An ordered map implementation that should perform effectively on all operations except
 * removal, where it performs linearly.
 */
class OrderedMap[A, B](map: Map[A, B] = Map[A, B](), queue: Queue[(A, B)] = Queue()) extends Map[A, B] {
